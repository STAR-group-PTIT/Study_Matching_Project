...
-- ============ focus_sessions (for Stats) ============
create table if not exists public.focus_sessions (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  room_id uuid, -- set if the session happened inside a shared room; FK added below once rooms exists
...
-- anyone signed in can see public rooms; host and members can also see private ones they belong to
...
-- ============ realtime ============
-- enable realtime broadcast on tables the Room screen needs to sync live
