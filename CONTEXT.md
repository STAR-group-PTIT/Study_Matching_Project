...
## Trạng thái hiện tại
**Giai đoạn 4 code xong, chờ user chạy migration + deploy Edge Function** (xem mục "Giai đoạn 4" bên dưới để biết việc cần làm thủ công). Auth thật, route guard, Dashboard to-do, Settings profile/Pomodoro defaults, và giờ Matching (tạo phòng/join mã/ghép ngẫu nhiên) + Room host tab (duyệt/kick) đều đã nối Supabase. Còn lại cho các giai đoạn sau: Stats vẫn dùng data mock (đúng kế hoạch — Giai đoạn 7), wallpapers/tracks CRUD vẫn local-only vì cần Supabase Storage (Giai đoạn 7), chat/nhạc/Pomodoro trong Room vẫn chưa đồng bộ realtime (Giai đoạn 5), video vẫn placeholder (Giai đoạn 6, LiveKit).
