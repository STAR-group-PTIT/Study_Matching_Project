...
- **2026-07-30 — User xác nhận đã chạy migration 0003.** Verify qua `javascript_tool` gọi thẳng REST API: `rooms` đã có đủ cột (`timer_phase`, `timer_round`, `timer_running`, `timer_remaining_seconds`, `timer_updated_at`, `music_on`, `music_track_index` — trả 200, có 1 row dữ liệu thật từ GĐ4), và `room_messages_view` trả 200 (rỗng, chưa có tin nhắn). Migration đã áp dụng đúng. **Vẫn chưa test được luồng thật 2 tài khoản** (Pomodoro đồng bộ, chat, nhạc, presence) — cần user tự test bằng 2 tài khoản đã xác nhận email, giống ghi chú "Chưa test được" ở trên.
